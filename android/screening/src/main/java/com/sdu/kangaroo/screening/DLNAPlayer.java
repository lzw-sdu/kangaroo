...
import com.sdu.kangaroo.screening.listener.OnRequestMediaProjectionResultCallback;
import com.sdu.kangaroo.screenrecorder.IRecorderCallback;
import com.sdu.kangaroo.screenrecorder.IScreenRecorderService;
import com.sdu.kangaroo.screenrecorder.Notifications;
import com.sdu.kangaroo.screenrecorder.ScreenRecorderServiceImpl;
...
public class DLNAPlayer {
    ...
    //    private RTMPPublish rtmpPublish;
    ...
    private void prepareMediaProjection() {
        if (null != mScreenRecorderService) {
            ...
            DisPlayActivity.callback = mRecorderCallback;
            DisPlayActivity.start(mContext);

//            RequestMediaProjectionActivity.resultCallback = mRequestMediaProjectionResultCallback;
//            RequestMediaProjectionActivity.start(mContext, mNotifications.getNotification(0));
...
//            String sourceUrl = mStreamMuxer.getMediaPath();
            String sourceUrl = "rtsp://" + DLNAManager.getLocalIpStr(mContext) +
...
//            mNotifications.recording(time);
        ...
        @Override
        public void onMuxAudio(ByteBuffer h264Buffer, MediaCodec.BufferInfo bufferInfo) {
...
//                mStreamMuxer.writeAudio(buffer, offset, length, bufferInfo);
        ...
        @Override
        public void onMuxVideo(ByteBuffer accBuffer, MediaCodec.BufferInfo bufferInfo) {
            if (null != mStreamMuxer) {
//                mStreamMuxer.writeVideo(buffer, offset, length, bufferInfo);
    ...
    private void startMirror(final @NonNull DLNAControlCallback callback) {
        ...
        if (displayService == null) {
            mContext.startService(new Intent(mContext, DisplayService.class));
        }
