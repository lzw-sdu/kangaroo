...
import com.sdu.kangaroo.screening.log.AndroidLoggingHandler;
...
public final class DLNAManager {
    ...
    public void init(@NonNull Context context, @Nullable DLNAStateCallback stateCallback) {
        ...
        //rtmp流媒体服务器
        new Thread(() -> {
            RTSPClientHelper.installAndStartRTSPServer(context);
        }).start();
        initLocalMediaServer();
        initConnection();
...
//            new Thread(RTSPClientHelper::startRTSPServer).start();
