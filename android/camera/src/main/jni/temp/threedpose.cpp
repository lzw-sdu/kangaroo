...
int Threedpose::load(const char *modeltype, int _target_size, const float *_mean_vals,
...
int
Threedpose::load(AAssetManager *mgr, const char *modeltype, int _target_size, const float *_mean_vals,
              const float *_norm_vals, bool use_gpu) {
    poseNet.clear();
    blob_pool_allocator.clear();
    workspace_pool_allocator.clear();
    ...
    mean_vals[1] = _mean_vals[1];
    mean_vals[2] = _mean_vals[2];
    mean_vals[3] = _mean_vals[3];
    norm_vals[0] = _norm_vals[0];
    norm_vals[1] = _norm_vals[1];
    norm_vals[2] = _norm_vals[2];
    norm_vals[3] = _norm_vals[3];
...
int Threedpose::detect(const cv::Mat &rgb) {
    //TODO:add person detection
    return 0;
}

void Threedpose::detect_pose(cv::Mat &rgb, std::vector<keypoint3d> &points) {
    int w = rgb.cols;
    int h = rgb.rows;
    float scale = 1.f;
    if (w > h) {
    ...
    ncnn::Mat in = ncnn::Mat::from_pixels_resize(rgb.data, ncnn::Mat::PIXEL_RGB, rgb.cols, rgb.rows,
                                                 w, h);
...
//        top_index = 0;
//        top_score = 0;
//        top_index = int(argmax(scores_data + feature_size * feature_size * i,
//                               scores_data + feature_size * feature_size * (i + 1)));
//        top_score = *std::max_element(scores_data + feature_size * feature_size * i,
//                                      scores_data + feature_size * feature_size * (i + 1));
//
...
//    points.clear();
//    for (int i = 0; i < num_joints; i++) {
//        float kpt_offset_x = offset_data[kpts_ys[i] * feature_size * num_joints * 2 +
//                                         kpts_xs[i] * num_joints * 2 + i * 2];
//        float kpt_offset_y = offset_data[kpts_ys[i] * feature_size * num_joints * 2 +
...
//        kpt.score = heatmap_data[kpts_ys[i] * feature_size * num_joints + kpts_xs[i] * num_joints +
//                                 i];
...
int Threedpose::draw(cv::Mat &rgb) {
    std::vector<keypoint3d> points;
    detect_pose(rgb, points);

//    int skele_index[][2] = {{0,  1},
//                            {0,  2},
//                            {1,  3},
//                            {2,  4},
//                            {0,  5},
...
//            std::string i_s = Convert(i);
//            std::string x_s = Convert(points[i].x);
//            std::string y_s = Convert(points[i].y);
//            std::string temp = i_s.append(" 、").append(x_s).append(" ").append(y_s);
//            LOG_D("%s", temp.c_str());
